...
func (t *Terminal) handleBlacklist(args []string) error {
	...
	if pn == 0 {
		...
		case "remove":
			...
			if err != nil {
				return fmt.Errorf("blacklist: %v", err)
			}
			log.Info("removed ip address from blacklist: %s", args[1])
			return nil
		}
	}
	return fmt.Errorf("invalid syntax: %s", args)
...
func (t *Terminal) createHelp() {
	...
	h.AddCommand("blacklist", "general", "manage automatic blacklisting of requesting ip addresses", "Select what kind of requests should result in requesting IP addresses to be blacklisted.", LAYER_TOP,
		readline.PcItem("blacklist", readline.PcItem("all"), readline.PcItem("unauth"), readline.PcItem("noadd"), readline.PcItem("off"), readline.PcItem("remove"), readline.PcItem("log", readline.PcItem("on"), readline.PcItem("off"))))
	...
	h.AddSubCommand("blacklist", []string{"remove"}, "remove <ip>", "remove an ip address from the blacklist")
